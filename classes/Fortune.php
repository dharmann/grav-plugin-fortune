...
class Fortune {
	...
	*/
	private function getNumberOfQuotes($file) {
	  $fd = fopen($file, "rb");
	...
	*/
	private function getExactQuote($file, $index) {
	...
	*/
	private function getRandomQuote($file) {
	...
	*/
	private function getQuote($fd, $index) {
		fseek($fd, $index);
		$line=""; $res = "";
		do {
			$res = $res . $line;
		...
		// Let the following parsing be optional and managed by settings
		{
			preg_match_all('/([\s\S]*)\n+--\s+(.*)$/m', $res, $matches);

			$quote = nl2br($matches[1][0]??$res);
			$signature = $matches[2][0]??'';

			$res  = "<h2>$quote</h2>";
			if($signature)
				$res .= "<p><em>$signature</em></p>";
		}
	...
	*/
	private function getIndices($fd) {
	...
	private function writeLong($fd, $l) {
